 ...
 * - [surge_http_api] 使用另一台设备上的 HTTP API. 设置后, 将不检测当前运行客户端, 并使用另一设备上的 HTTP API 执行请求. 默认不使用. 例: 192.168.31.5:6171
...
async function operator(proxies = [], targetPlatform, context) {
  ...
  let format = $arguments.format || '{{api.country}} {{cleanASN api.asname}} - {{proxy.name}}'
  let url = $arguments.api || 'http://ip-api.com/json/?fields=66846719'
  if (internal) {
    // if (isSurge) {
    //   //
    // } else if (isLoon) {
  ...
  function cleanASN(asname) {
    if (!asname) return 'Sector 2814';
    
  // 首先尝试匹配已知的 ASN，使用更宽松的匹配
  const knownASN = {
    'AKARI': 'Akari',
    'AKAMAI': 'Akamai',
    'AKILE': 'Akile',
    ...
    'BAXET': 'Baxet',
    'Kadir Huseyin': 'Nosspeed',
    'Melbikomas': 'MelbikomasUAB',
    'LightNode': 'LightNode'
  };

  for (const [key, value] of Object.entries(knownASN)) {
    if (asname.toUpperCase().includes(key)) {
      return value;
    }
  }
    
    // 如果使用 Cloudflare API 且 format 包含 cleanASN api.asOrganization
    if (url.includes('speed.cloudflare.com/meta') && format.includes('{{cleanASN api.asOrganization}}')) {
      // 取第一个空格或者_或者.Co.LTD之前的第一个单词
      let cleaned = asname.split(/[\s_]|\.(Co\.LTD)/)[0];
      
      // 将清理后的名称转换为首字母大写，其他字母小写的格式
      cleaned = cleaned.toLowerCase().replace(/^[a-z]/, (letter) => letter.toUpperCase());
      
      return cleaned || 'Sector 2814';
    }
    
    // 如果不是 Cloudflare API 或 format 不包含 cleanASN api.asOrganization，使用原来的清理逻辑
    let cleaned = asname
      .replace(/^AS-?/i, '')  // 移除开头的 AS- 或 AS（不区分大小写）
      .replace(/^NET-?/i, '') // 移除开头的 NET- 或 NET（不区分大小写）
      .replace(/-?(AP|AS|CN|NET)$/i, '');  // 移除结尾的 -AP, -AS, -CN, -NET（不区分大小写）
  ...
  function getBandwidthMultiplier(name) {
    const regexArray = [/[1-9]\d*x/i, /[1-9]\d*X/i, /[1-9]\d*倍/i, /ˣ[²³⁴⁵⁶⁷⁸⁹¹⁰]/];
    const valueMap = {
        '2': 'ˣ²', '3': 'ˣ³', '4': 'ˣ⁴', '5': 'ˣ⁵', '6': 'ˣ⁶', '7': 'ˣ⁷', '8': 'ˣ⁸', '9': 'ˣ⁹', '10': 'ˣ¹⁰',
        '20': 'ˣ²⁰', '30': 'ˣ³⁰', '40': 'ˣ⁴⁰', '50': 'ˣ⁵⁰'
    };
    ...
    for (const regex of regexArray) {
        ...
        if (match) {
            const value = match[0].toLowerCase().replace(/x|倍/, '');
            // 如果是 1 倍率，返回空字符串
            if (value === '1') {
                return '';
            }
            return valueMap[value] || `ˣ${value}`;
        }
    }

    return ''; // 默认倍率（包括未找到倍率的情况）
  }

  function formatter({ proxy = {}, api = {}, format = '' }) {
    return format.replace(/\{\{(.*?)\}\}/g, (match, p1) => {
      if (p1.trim().startsWith('cleanASN')) {
        const params = p1.trim().split(' ');
        const asnameValue = params[1].split('.').reduce((obj, prop) => obj && obj[prop], { proxy, api });
        return cleanASN(asnameValue);
      }
      if (p1.trim() === 'proxy.name.bl') {
        return getBandwidthMultiplier(proxy.name);
      }
      const props = p1.split('.');
      let value = props.reduce((obj, prop) => obj && obj[prop], { proxy, api });
